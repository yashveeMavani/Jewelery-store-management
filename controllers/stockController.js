...
// exports.updateStockFromSales = async (req, res) => {
//   try {
...
//       let existingStock = await StockData.findOne({ where: { stock_code: stockCode } });

//       if (!existingStock) {
//         console.error(`Stock entry not found for ${stockCode}, sale cannot be processed.`);
//         return res.status(400).json({ error: `Stock entry not found for ${stockCode}, sale cannot be processed.` }); // ⛔ EXIT FUNCTION IMMEDIATELY
//       }
...
//         console.error(`Not enough stock for ${stockCode}, sale exceeds available quantity.`);
//         return res.status(400).json({ error: `Not enough stock for ${stockCode}, sale exceeds available quantity.` }); // ⛔ EXIT FUNCTION IMMEDIATELY
//       }
...
//       stockUpdated = true;
//       console.log(`Stock updated after sale for ${stockCode}: Gross ${newGrossWeight}, Net ${newNetWeight}`);
//     }

//     // If no stock was updated, send an error response
//     if (!stockUpdated) {
//       return res.status(400).json({ error: "No stock updates were performed." });
//     }

//     // Send success response only once at the end
//     return res.json({ message: "Stock data updated after sales successfully!" });

//   } catch (error) {
//     console.error('Error updating stock data from sales:', error);
//     return res.status(500).json({ error: error.message }); // ⛔ EXIT FUNCTION IMMEDIATELY
//   }
// };

// exports.updateStockFromSales = async (req) => {  // Remove `res`
//   try {
//     const salesSummary = await SalesOrder.findAll({
//       attributes: [
//         'category',
